...
import { checkStorageHealth, getStorageManager, StorageHealthInfo } from '../db/database';
...
/**
 * Storage quota hook options
 */
export interface UseStorageQuotaOptions {
  checkInterval?: number;
  autoCheck?: boolean;
  onWarning?: (health: StorageHealthInfo) => void;
  onCritical?: (health: StorageHealthInfo) => void;
  onCleanupComplete?: (results: CleanupResults) => void;
}

/**
 * Return type for useStorageQuota hook
 */
export interface UseStorageQuotaReturn {
  // State
  storageInfo: StorageHealthInfo | null;
  isLoading: boolean;
  error: string | null;
  ...
  // Actions
  checkStorage: () => Promise<StorageHealthInfo | null>;
  performCleanup: (options?: CleanupOptions) => Promise<CleanupResults | null>;
  performAutoCleanup: () => Promise<CleanupResults | null>;
...
export function useStorageQuota(options: UseStorageQuotaOptions = {}): UseStorageQuotaReturn {
  ...
  // Refs for callbacks
  const onWarningRef = useRef<typeof onWarning>(onWarning);
  const onCriticalRef = useRef<typeof onCritical>(onCritical);
  const onCleanupCompleteRef = useRef<typeof onCleanupComplete>(onCleanupComplete);
  ...
  // Check storage quota
  const checkStorage = useCallback(async (): Promise<StorageHealthInfo | null> => {
    try {
      setError(null);
  ...
  // Perform cleanup
  const performCleanup = useCallback(async (options: CleanupOptions = {}): Promise<CleanupResults | null> => {
    const {
      cleanSyncQueue = true,
      cleanOldData = true,
      cleanOrphaned = true,
  ...
  const isStoragePersisted = useCallback(async (): Promise<boolean> => {
    try {
      const storageManager = getStorageManager();
  ...
  const performAutoCleanup = useCallback(async (): Promise<CleanupResults | null> => {
  ...
  useEffect(() => {
    const handleStorageWarning = (event: Event): void => {
      checkStorage();
      if (onWarningRef.current && event instanceof CustomEvent) {
        onWarningRef.current(event.detail);
      }
    };

    const handleStorageCritical = (event: Event): void => {
      checkStorage();
      if (onCriticalRef.current && event instanceof CustomEvent) {
