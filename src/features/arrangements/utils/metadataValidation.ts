...
export interface AllMetadataValidationResult {
  valid: boolean;
  errors: Record<string, string>;
}

export interface ArrangementMetadataInput {
  key?: string;
  tempo?: number | string;
  capo?: number | string;
 ...
 */
export function validateTempo(value: number | string | null | undefined): ValidationResult {
 ...
 * @param value - Capo fret number
 * @returns Validation result
 */
export function validateCapo(value: number | string | null | undefined): ValidationResult {
  if (value === null || value === undefined || value === '') {
 ...
 * @param value - Key name (e.g., 'C', 'G', 'Am')
 * @returns Validation result
 */
export function validateKey(value: string | null | undefined): ValidationResult {
  if (!value || value.trim() === '') {
 ...
 * @param value - Time signature (e.g., '4/4', '3/4')
 * @returns Validation result
 */
export function validateTimeSignature(value: string | null | undefined): ValidationResult {
  if (!value || value.trim() === '') {
    return { valid: true, error: null };  // Optional field
  }
